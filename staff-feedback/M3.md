...
You implemented multiple meaningful epics, from the user profile to the discovery of people nearby, the meeting request and the suggestions for the first encounter.
...
The features you implemented are usable and fun for the users to try, with a reasonable complexity to make. Great job!
...
Your app is working well, we haven't seen it crash. It can have some unexpected behavior, for example we were able after reporting someone to see our own profile in the nearby people and send a meeting request to ourself. We however not received it. The UI could be more fluid without all the animations but it is still usable.

You also have a great account management, with the possibility of blocking and reporting other accounts as well as managing you own profile and deleting it. This is very GDPR friendly. Good job!


For this part, you received 6.5 points out of a maximum of 8.0.
...
Your app is rather intuitive, you reassure the users with the different steps of your app. It can be even more smooth to use.
...
You achieved a descent coverage but you still struggle with the SonarCloud reporting, which prevent us to see the right coverage for each PR. This could be improved. Otherwise the rest of the CI with the build mechanism is good.
...
You have a good README, Wiki and architecture diagram, we can understand your project quite well. You could improve the figma to be more exhaustive on the interactions that are in the app, showing more in depth the principles of the encounter and also add a few screens to plan ahead what would be future work.
...
Based on the above points, your intermediate grade for this milestone M3 is 5.20.
