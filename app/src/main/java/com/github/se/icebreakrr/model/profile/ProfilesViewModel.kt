...
open class ProfilesViewModel(
    ...
    /**
     * Checks internet connection after a delay
     * If still no connection after 15 seconds, updates waitingDone state
     */
    ...
    fun getFilteredProfilesInRadius(
        ...
        repository.getProfilesInRadius(
            ...
            onSuccess = { profileList ->
                ...
                Log.e("ConnectionCheck", "Reset flags: waiting=${isWaiting.value}, done=${waitingDone.value}")
            },
            onFailure = { e ->
                Log.e("ConnectionCheck", "Request FAILED")
                Log.e("ConnectionCheck", "Current state: waiting=${isWaiting.value}, done=${waitingDone.value}")
                handleError(e)
