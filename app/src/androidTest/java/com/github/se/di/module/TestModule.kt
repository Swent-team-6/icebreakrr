...
object MockFirebaseFirestoreModule {
  ...
  private fun provideMockFirebasefirestore(): FirebaseFirestore {
    ...
    for (i in 0 until 12) {
      `when`(mockCollectionReferenceProfiles.document(i.toString()))
      ...
      doAnswer { invocation ->
            ...
            listener.onComplete(
                profilesTaskDocumentSnapshot[
                    i]) // Trigger the listener with the mockTask, simulating success
    ...
    doAnswer { invocation ->
          val listener = invocation.arguments[0] as OnCompleteListener<DocumentSnapshot>
          listener.onComplete(
              mockMyTaskDocumentSnapshot) // Trigger the listener with the mockTask, simulating
                                          // success
    ...
    doAnswer { invocation ->
          ...
          // re-update mocking of my profile after set changes
          `when`(mockMyDocumentSnapshot.getString("name")).thenReturn(myProfile.name)
          `when`(mockMyDocumentSnapshot.getString("gender")).thenReturn(myProfile.gender.toString())
          `when`(mockMyDocumentSnapshot.getTimestamp("birthDate")).thenReturn(myProfile.birthDate)
          `when`(mockMyDocumentSnapshot.getString("catchPhrase")).thenReturn(myProfile.catchPhrase)
    ...
    doAnswer { invocation ->
          ...
          listener.onComplete(
              mockVoidTask) // Trigger the listener with the mockTask, simulating success
          mockVoidTask
        }
        .`when`(mockVoidTask)
        .addOnCompleteListener(any())
